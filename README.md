...
O modo de zoom interativo, opção 3 do menu, é iniciado após verificar que existe uma imagem carregada no sistema. O programa exibe um submenu que permite ao usuário selecionar qual tipo específico de zoom deseja aplicar. Em vez de alternância automática, o sistema apresenta opções claras para cada algoritmo disponível. Para operações de zoom in (ampliação), o usuário pode escolher entre o algoritmo de vizinho mais próximo ou o algoritmo de replicação de pixels. Para operações de zoom out (redução), as opções disponíveis são o algoritmo de média ou o algoritmo de decimação.
